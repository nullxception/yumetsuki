...
constructor(
    ...
    private fun updateNotifier(transform: suspend (NotifierSettings) -> NotifierSettings) {
        viewModelScope.launch { settingsRepo.updateNotifier(transform) }
    }

    fun notifyOnCheckInSuccess(enabled: Boolean) = updateNotifier {
        it.copy(onCheckInSuccess = enabled)
    }

    fun notifyOnCheckInFailed(enabled: Boolean) = updateNotifier {
        it.copy(onCheckInFailed = enabled)
    }

    fun notifyOnResin(option: ResinOption) = updateNotifier { it.copy(onResin = option) }

    fun notifyOnRealmCurrencyFull(enabled: Boolean) = updateNotifier {
        it.copy(onRealmCurrencyFull = enabled)
    }

    fun notifyOnExpeditionCompleted(enabled: Boolean) = updateNotifier {
        it.copy(onExpeditionCompleted = enabled)
    }
}
